  ...
  while (mcp2515.readMessage(&canRx) == MCP2515::ERROR_OK) {
    switch (canRx.can_id) {
      ...
      case RSCP_ID_ENV_TELEM:
        mast.temp_x10  = (int16_t)rscp_le_to_u16(&canRx.data[0]);
        mast.humidity  = canRx.data[2];
        alive_m2 = true;
        break;

      case RSCP_ID_RF_TELEM:
        mast.vbat_raw   = rscp_le_to_u16(&canRx.data[0]);
        mast.vswr_state = canRx.data[2];
        mast.drive_state= canRx.data[3];
        break;

      case RSCP_ID_HEADING:
        compass_deg = rscp_le_to_u16(&canRx.data[0]);
        alive_m3 = true;
        break;

      case RSCP_ID_ROT_STATUS:
        rotator_deg = rscp_le_to_u16(&canRx.data[0]);
        alive_m4 = true;
        break;

      case RSCP_ID_CFG_ACK:
        if (canRx.data[0] == RSCP_NODE_M2) cfgAckM2 = canRx.data[1];
        if (canRx.data[0] == RSCP_NODE_M3) cfgAckM3 = canRx.data[1];
        if (canRx.data[0] == RSCP_NODE_M4) cfgAckM4 = canRx.data[1];
        break;
    }
  }
}
