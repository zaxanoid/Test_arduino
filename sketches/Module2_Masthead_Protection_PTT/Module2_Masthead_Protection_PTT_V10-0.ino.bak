...
// PTT confirmation policy (set by station via config; default disabled)
static bool require_ptt_confirm = false;
...
void loop() {
  ...
  if (ptt_active_hw) {
    ...
    // overtime enforcement (always active when PTT is ON)
