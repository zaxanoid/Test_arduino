...
// ======================================================
// ===== DEBUG SERIAL (TEMP) =============================
// Comment out or set to 0 when finished debugging.
// ======================================================
#define RSCP_DEBUG_SERIAL 1
...
static void dbgPrintFrame(const char* tag, uint16_t id, const uint8_t* d, uint8_t len) {
#if RSCP_DEBUG_SERIAL
  DBG_PRINT(tag);
  DBG_PRINT(" id=0x"); DBG_PRINT_HEX(id);
  DBG_PRINT(" len="); DBG_PRINT(len);
  DBG_PRINT(" data=");
  for (uint8_t i = 0; i < len; i++) {
    if (d[i] < 16) DBG_PRINT('0');
    DBG_PRINT_HEX(d[i]);
    DBG_PRINT(' ');
  }
  DBG_PRINTLN("");
#else
  (void)tag; (void)id; (void)d; (void)len;
#endif
}
